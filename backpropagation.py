...
def out_grad_weights_w2(Z2, W2, d_out, reg = 1):
    UpdW2 = d_out.T.dot(Z2) + np.multiply(reg, W2) # Z is the output of the hidden layer and d_out the error_output
    return UpdW2
...
def backpropagate(X, Y, T, Z1, Z2, W1, W2, actFunction):
    d_out = error_output(Y, T)
    UpdW2 = out_grad_weights_w2(Z2, W2, d_out)
